...
def _multi_view_output_path(output_dir: pathlib.Path, suffix: str, index: int, elev: float, azim: float) -> pathlib.Path:
    return output_dir / f"{index:04d}_elev_{_format_angle(elev)}_azim_{_format_angle(azim)}{suffix}"


def _format_multiview_report(config, rows: list[tuple[int, float, float, pathlib.Path]]) -> str:
    lines = [
        "Multi-View Render Report",
        f"input: {config.input}",
        f"resolution: {config.resolution}",
        f"render_mode: {config.render_mode}",
        f"chunk_size: {config.multi_view_chunk_size}",
        f"view_count: {len(rows)}",
        "",
        "index | elev | azim | output",
    ]
    for index, elev, azim, output_path in rows:
        lines.append(f"{index:04d} | {elev:.3f} | {azim:.3f} | {output_path}")
    return "\n".join(lines) + "\n"
...
def _render_multiview_from_config(config) -> tuple[list[pathlib.Path], pathlib.Path]:
    if config.render_all:
        raise ValueError("--render-all is not supported together with multi-view ranges")
    if config.display:
        print("Ignoring --display during multi-view rendering.")
    view_pairs = _multi_view_pairs(config)
    output_dir = _render_all_output_dir(config.output)
    output_dir.mkdir(parents=True, exist_ok=True)
    suffix = _render_all_suffix(config.output)
    ...
    chunk_count = (len(view_pairs) + chunk_size - 1) // chunk_size
    indexed_pairs = list(enumerate(view_pairs))
    for chunk_index, chunk_start in enumerate(range(0, len(indexed_pairs), chunk_size), start=1):
        chunk_pairs = indexed_pairs[chunk_start: chunk_start + chunk_size]
        print(f"Rendering multi-view chunk {chunk_index}/{chunk_count} ({len(chunk_pairs)} view(s))")
        jobs = []
        for view_index, (elev, azim) in chunk_pairs:
            view_config = replace(base_config, elev=elev, azim=azim)
            output_path = _multi_view_output_path(output_dir, suffix, view_index, elev, azim)
            ...
            rows.append((view_index, elev, azim, output_path))
        try:
    ...
    report = _format_multiview_report(config, rows)
...
def render_multi_view() -> tuple[list[pathlib.Path], pathlib.Path]:
    ...
    if not _is_multi_view(config):
        raise ValueError("At least one full multi-view range triplet is required")
