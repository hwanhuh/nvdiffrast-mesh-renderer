...
class SceneRenderer:
    ...
    def prepare_scene(self, input_path: pathlib.Path) -> PreparedScene:
        ...
        camera = self.scene_builder.build_camera(meshes, self.config)
        ...
        bg_rgb, bg_alpha = self.environment.render_background(camera, self.config, env, self.device)
        return PreparedScene(meshes=meshes, camera=camera, lights=lights, ibl=ibl, bg_rgb=bg_rgb, bg_alpha=bg_alpha)
